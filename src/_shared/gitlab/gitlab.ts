...
const fetchProjectsPage = (
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        ...
        if (!hasResponse) {
            ...
            _log.FAIL(
                `No valid response for ${target}, returning collected items...`
            );
            return null;
        }
        ...
        if (!projects) {
            ...
            _log.FAIL(
                `Invalid JSON for ${target}, returning collected items...`
            );
            return null;
        }
...
export const getAllProjects = (
    ...
    LOG.OK(`Max available items: ${maxAvailableItems}`);

    if (isNaN(maxAvailablePages)) {
        _log.WARN(
            `No xTotalPages header found in response from ${firstPage.target}`
        );
        return {
            items: [],
            logger: _log,
    ...
    for (let page = nextPage; page > 0 && page <= maxPages; page++) {
        ...
        if (!pageResult) {
            _log.WARN(
                `Failed to fetch page ${page}, stopping further requests.`
            );
            break;
        }
